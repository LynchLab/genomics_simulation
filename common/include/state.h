...
#define LZ4_BUFFER_SIZE	200000000
...
//FUNCTIONS
	...
	{
		size_=rhs.size_;
		std::cerr << "setting private " << std::endl;
		sites_=rhs.sites_;
...
public:
	...
	void uncompress (uint32_t *a, uint32_t *b, const uint32_t &k, State_stream &) const;
	...
	void cache (void);
	void rewind (void);
	void rewind (State_stream &) const;
	void advance (void);
	void finalize (void);
...
State rand_drop(const State &, const float &, std::mt19937 &);
State rand_err(const State &, const float &, std::mt19937 &);
