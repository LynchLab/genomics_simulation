...
public:
	...
	//friend C_it<T> operator+(const int &rhs); 

	C_it next (void)
	{
		return C_it(itr_->next);
	}

	C_it  
	operator+ (const int &rhs) // addition
	{
		C_it result(*this);
		for (int x=0; x<rhs; ++x) result.itr_ = result.itr_->next;
		return result;
	}
	...
	{
		C_it result(*this);
		for (int x=0; x<rhs; ++x) result.itr_ = result.itr_->prev;
		return result;
	}
	C_it & 
	operator-- (void) // Pre-increment
...
public:
	...
	Circular_list(const T &val)
	{
		...
		//std::cout << "prev is now " << origin_.itr_->prev->value << std::endl;
	}
