...
int main (int argc, char **argv){
	...
	env.optional_arg('G',"vg",  H2,      "please provide a number.", "Genotypic variance.");
	...
	/*
	MATRIX E=MATRIX::Zero(2*N,6);	
	MATRIX ET=MATRIX::Zero(2*N, 1);	
	MATRIX EG=MATRIX::Zero(2*N, 1);	
	MATRIX ED=MATRIX::Zero(2*N, 1);	
	MATRIX EN=MATRIX::Zero(2*N, 1);	
	*/
		...
		for (size_t k=0; k<WORD; k++)
		{
			...
			if (p>0 and p<1)
			{
				...
				if ( E(p_arr[k],0)==0)
				{
					...
					E(p_arr[k],0) = ET * ( (sa)+(sd)*powf(q-p, 2)+2*(sad)*(q-p) );
					E(p_arr[k],1) = ED * (sd);
					E(p_arr[k],2) = ET * (sd)*powf(q-p, 2);
               		                E(p_arr[k],3) = EG * (q-p)*(sd);
					E(p_arr[k],4) = EG * ( (q-p)*(sd)+(sad) );
					E(p_arr[k],5) = ET * (-2*powf(q-p, 2)*(sd) -2*(sad)*(q-p) );
				} else {
					A1 += E(p_arr[k],0);
					B1 += E(p_arr[k],1);
					B2 += E(p_arr[k],2);
	...
	if (H2!=1)
	{
		TYPE NORM=(S4T+S5T+S6T)/H2;
		std::cerr << NORM << std::endl;
		std::cerr << "Var(Beta)/Var(Z)\t" << S4T/NORM << std::endl; 
		std::cerr << "Var(Delta)/Var(Z)\t" << S5T/NORM << std::endl;
		std::cerr << "2*Cov(Beta,Delta)/Var(Z)\t" << S6T/NORM << std::endl;
	} else  {
