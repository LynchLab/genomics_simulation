...
#	global F
	...
	a, d=numpy.random.multivariate_normal([A,D], ( (1,AD),(AD, 0.75) ), 1).T
	a=a[0]
	d=d[0]
#	print a, d
	g=[-a,d,a]
...
N=len(name)
SIZE=int(sys.argv[2])	#Number of loci in the genome
LOCI=int(sys.argv[3])	#Number of causative loci
...
betas=[]
deltas=[]
for x in range(0, len(pop.d_all[0]) ):
	betas.append("beta_"+str(x) )
	deltas.append("delta_"+str(x) )
print "X,Y,"+','.join(betas)+","+','.join(deltas)
for x in range(0, N):
...
#sys.stderr.write("x: "+str(LOCI)+", A: "+str(numpy.var(pop.a))+", D:"+str(numpy.var(pop.d))+", AD:"+str(numpy.cov(pop.d, pop.a)[0,1])+", cov(F,z):"+str(numpy.cov(F, pop.g)[0,1] )+", a:"+str(A)+", d:"+str(D)+"cov(a,d):"+str(AD)+'\n')
if (numpy.var(pop.g)>0):
	sys.stderr.write(str(LOCI)+", "+str(numpy.var(pop.a)/numpy.var(pop.g))+'\n')
else:
	sys.stderr.write(str(LOCI)+", NaN"+'\n')
