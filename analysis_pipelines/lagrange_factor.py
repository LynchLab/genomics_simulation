...
#DATA
								# A	B   C	  D   E     F   G
A, B, C, D, E, F, G, trA, trB, trC, trD, trE, trF, trG = symbols("r.MtM MJM r.HtH HJH r.MtH MJH v trMtM trMJM trHtH trHJH trMtH trMJH trE")
#parameters
...
obj = A*a -2* (a*b**2)/(a + b**2)*B +C*c -2*(c*d**2)/(c + d**2) * D+(e * E - 2*e*b*d/(e+b*d)*F )+f*G-l*(1.-a*trA-c*trC-e*trE-f*trG+2*(a*b**2)/(a + b**2)*trB+2*(c*d**2)/(c + d**2)*trD+2*e*b*d/(e+b*d)*trF )
...
for x in range (0,7):
	s1=all_as[x]
	R=collect(obj.diff(s1), [A, B, C, D, E, F])
	if(R!=0):
		print "\tcase("+str(x)+"):"
		print "\t{"
		if B in R.free_symbols:
			print "\t\tMJM=r.Mt1*r.Mt1.transpose();"
		if D in R.free_symbols:
			print "\t\tHJH=r.Ht1*r.Ht1.transpose();"
		if F in R.free_symbols:
...
for x in range (0,7):
	...
	for y in range (x,7):
		...
		R=collect(obj.diff(s1,s2), [A, B, C, D, E, F])
